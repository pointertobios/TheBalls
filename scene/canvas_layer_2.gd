...
@onready var spike_control: Control = $Control  # 引用 Control 节点
...
func _process(delta: float) -> void:
	...
	# 检测 E 键是否被按下
	if Input.is_action_just_pressed("ui_e") and is_ultimate_ready and not is_ultimate_active:
...
func start_ultimate_animation() -> void:
	...
	spike_control.trigger_spike_animation()  # 触发尖刺动画
