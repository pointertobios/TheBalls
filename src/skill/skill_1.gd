...
@export var cooldown_time: float = 5.0  # 冷却时间
...
func _process(delta: float):
	# 检测是否按下 E 键
	if Input.is_action_just_pressed("KEY_E"):  # 假设 "use_skill" 是 E 键的输入动作
...
func use_skill():
	# 技能使用的逻辑
	print("Skill used!")  # 这里可以替换为实际的技能逻辑
